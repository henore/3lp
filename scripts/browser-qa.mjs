...
for(const width of [375,1440]){
 ...
 for(const locale of locales){
  ...
  if(locale==='ja')await page.screenshot({path:'test-results/ja-'+width+'.png',fullPage:true});
  results.push(result);
 }
}
await page.goto('http://127.0.0.1:4321/ja/');
...
await page.getByRole('link',{name:'عن التطبيق',exact:true}).first().click();
...
await writeFile('test-results/browser.json',JSON.stringify({results,errors},null,2));
